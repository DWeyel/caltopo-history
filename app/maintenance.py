...
def tool_data_size() -> int:
    # Count only files managed by this application, not unrelated files that might share a custom DB directory.
...
def map_storage(db: Session, map_id: str) -> MapStorage:
    ...
    return MapStorage(
        map_id=map_id,
        title=(watch.title if watch and watch.title else map_id),
...
def storage_overview(db: Session) -> StorageOverview:
    ...
    def grouped(model, blob_column):
        rows = db.execute(
            select(model.map_id, func.count(model.id), func.coalesce(func.sum(func.length(blob_column)), 0))
            .group_by(model.map_id)
        ).all()
        return {str(mid): (int(count or 0), int(size or 0)) for mid, count, size in rows}

    snapshots = grouped(Snapshot, Snapshot.state_gz)
    versions = grouped(ObjectVersion, ObjectVersion.feature_gz)
    current = grouped(CurrentObject, CurrentObject.feature_gz)
    map_ids = set(watches) | set(snapshots) | set(versions) | set(current)

    rows: list[MapStorage] = []
    for map_id in sorted(map_ids, key=lambda mid: ((watches.get(mid).title if watches.get(mid) else "") or mid).lower()):
        watch = watches.get(map_id)
        snapshot_count, snapshot_bytes = snapshots.get(map_id, (0, 0))
        version_count, version_bytes = versions.get(map_id, (0, 0))
        current_count, current_bytes = current.get(map_id, (0, 0))
        rows.append(MapStorage(
            map_id=map_id,
            title=(watch.title if watch and watch.title else map_id),
            watched=watch is not None,
            snapshot_count=snapshot_count,
    ...
    return StorageOverview(
        maps=rows,
        snapshot_count=sum(r.snapshot_count for r in rows),
        snapshot_bytes=sum(r.snapshot_bytes for r in rows),
        version_count=sum(r.version_count for r in rows),
        version_bytes=sum(r.version_bytes for r in rows),
        current_count=sum(r.current_count for r in rows),
        current_bytes=sum(r.current_bytes for r in rows),
        database_bytes=database_files_size(),
        database_backup_bytes=sum(item.size for item in database_backup_files()),
        tool_data_bytes=tool_data_size(),
        database_backups=database_backup_files(),
    )


def prune_snapshots(db: Session, *, map_id: str | None, older_than_days: int, keep_latest: int) -> int:
    """Delete old snapshots while always retaining at least `keep_latest` snapshots per map."""
    ...
    for mid in map_ids:
        snapshots = db.scalars(
            select(Snapshot).where(Snapshot.map_id == mid).order_by(Snapshot.captured_at.desc(), Snapshot.id.desc())
        ).all()
        ...
        for snap in snapshots[keep_latest:]:
            ...
            if captured < cutoff and snap.id not in keep_ids:
                db.delete(snap)
                deleted += 1
    db.commit()
    return deleted
...
def prune_object_versions(db: Session, *, map_id: str | None, older_than_days: int) -> int:
    """Delete old object history while retaining the newest version of every object.

    CurrentObject is not touched, so the current reconstructed map state remains intact.
    """
    if older_than_days < 1:
        raise ValueError("older_than_days must be >= 1")
    cutoff = utcnow() - timedelta(days=older_than_days)
    stmt = select(ObjectVersion)
    if map_id:
        stmt = stmt.where(ObjectVersion.map_id == map_id)
    rows = db.scalars(stmt.order_by(ObjectVersion.map_id, ObjectVersion.object_id, ObjectVersion.id.desc())).all()
...
def purge_archived_map(db: Session, map_id: str) -> dict[str, int]:
    """Permanently remove stored history for a map that is no longer watched."""
    if db.scalar(select(MapWatch).where(MapWatch.map_id == map_id)) is not None:
        raise ValueError("Map is still watched")
    counts = {
        "snapshots": db.scalar(select(func.count()).select_from(Snapshot).where(Snapshot.map_id == map_id)) or 0,
        "versions": db.scalar(select(func.count()).select_from(ObjectVersion).where(ObjectVersion.map_id == map_id)) or 0,
        "current": db.scalar(select(func.count()).select_from(CurrentObject).where(CurrentObject.map_id == map_id)) or 0,
    }
    db.execute(delete(Snapshot).where(Snapshot.map_id == map_id))
    db.execute(delete(ObjectVersion).where(ObjectVersion.map_id == map_id))
    db.execute(delete(CurrentObject).where(CurrentObject.map_id == map_id))
    db.commit()
...
def vacuum_database() -> None:
    ...
    if not settings.database_url.startswith("sqlite"):
        raise ValueError("VACUUM is only implemented for SQLite")
