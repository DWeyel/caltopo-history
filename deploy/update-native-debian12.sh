...
if [ ! -d "$APP_DIR" ] || [ ! -f "$ENV_FILE" ]; then
    echo "Existing native installation not found. Use install-native-debian12.sh instead." >&2
    exit 1
fi

# Refuse the updater backup if it would consume the last safety reserve on the filesystem.
if [ -f "$DB_FILE" ]; then
...
PYDB
    ...
    if [ "$FREE_BYTES" -lt "$REQUIRED_BYTES" ]; then
        ...
        echo "Free bytes: $FREE_BYTES; required: $REQUIRED_BYTES (database copy + ${HARD_MB} MiB reserve)" >&2
        exit 1
    fi
fi
...
if [ -f "$DB_FILE" ]; then
    cp -a "$DB_FILE" "$DB_FILE.pre-v1.0-$STAMP.bak"
    chown caltopo-history:caltopo-history "$DB_FILE.pre-v1.0-$STAMP.bak"
    chmod 0600 "$DB_FILE.pre-v1.0-$STAMP.bak"
    echo "Database backup: $DB_FILE.pre-v1.0-$STAMP.bak"
fi
...
echo
echo "Update to CalTopo History v1.0 complete. Existing /etc/caltopo-history.env was not changed."
